...
import sys
sys.path.append(".")
from kcc.models.psb import PSBGPT2LMHeadModel
from kcc.datasets.timeseries_datasets import PSBMDataset
...
def main(
    ...
    save_dir = f"./data/hub/{data}-{overlap_}-{patch_length}-{n_prototype}-{context_size}/psb-{n_layer}-{n_head}-{attn_}"
    
    dataset = PSBMDataset(data_file=os.path.join(root_path, processed_data_file),
                                  context_size=context_size,
                                  type="train")
    
    model = PSBGPT2LMHeadModel(n_layer=n_layer,
                            n_head=n_head,
